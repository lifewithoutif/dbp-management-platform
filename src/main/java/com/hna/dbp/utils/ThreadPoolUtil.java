...
public final class ThreadPoolUtil {
	...
	private transient static ExecutorService executor = new ThreadPoolExecutor(TWICE_CPU_NUM, TWICE_CPU_NUM, 5L,
			TimeUnit.SECONDS, new LinkedBlockingQueue<Runnable>(),
			new NamingThreadFactory(new DaemonThreadFactory(), "dbp"));
	...
	public static void restart() {
		...
		executor = new ThreadPoolExecutor(TWICE_CPU_NUM, TWICE_CPU_NUM, 5L, TimeUnit.SECONDS,
				new LinkedBlockingQueue<Runnable>(), new NamingThreadFactory(new DaemonThreadFactory(), "Jenkins"));
	}
	 ...
	 * 
	 * @see ThreadPoolUtil#sleep(Number)
	 * @param millis
	 *            给定的sleep时间
	 * @return 被中断返回false，否则true
	 */
